...
var (
	...
	unblock = flag.Bool("unblock", false, "remove flag of wallet_id instead")
...
func main() {
	flag.Parse()
	if *wallet == "" {
		fmt.Println("cannot block wallet_id ''")
		os.Exit(1)
	}
	...
	err = emitter.EmitSync(*wallet, &pb.FlagEvent{FlagRemoved: *unblock})
	if err != nil {
		panic(err)
	}
}
